...
import {
  ...
  Row, Col, List, Drawer, Descriptions, Divider,
} from 'antd';
import {
          ...
          <Col span={8}>
            ...
            >
              <Spin spinning={loading}>
                {docs.length === 0 ? (
                  <Empty description="暂无知识文档" image={Empty.PRESENTED_IMAGE_SIMPLE} />
                ) : (
                  <List
                    dataSource={docs}
                    renderItem={(d) => (
                      <List.Item
                        ...
                        style={{
                          ...
                          background: selectedKey === d.doc_key ? '#e6f4ff' : undefined,
                          border: selectedKey === d.doc_key ? '1px solid #91caff' : '1px solid transparent',
                          marginBottom: 4, opacity: d.active ? 1 : 0.5,
                        }}
                        actions={
                          isAdmin ? [
                            d.active ? (
                              <Popconfirm key="disable" title={`停用「${d.doc_key}」？`} onConfirm={(e) => { e?.stopPropagation(); handleDisable(d.doc_key); }} onCancel={(e) => e?.stopPropagation()} okText="确认" cancelText="取消">
                                <Button type="link" size="small" danger icon={<StopOutlined />} onClick={(e) => e.stopPropagation()} />
                              </Popconfirm>
                            ) : (
                              <Button key="enable" type="link" size="small" icon={<CheckCircleOutlined />} style={{ color: '#52c41a' }} onClick={(e) => { e.stopPropagation(); handleEnable(d.doc_key); }} />
                            ),
                          ] : undefined
                        }
                      >
                        <List.Item.Meta
                          title={<Space>{d.title}<Tag color={d.active ? 'success' : 'default'}>{d.active ? '启用' : '停用'}</Tag></Space>}
                          description={<span style={{ fontSize: 12, color: '#999' }}>{d.doc_key} · {d.chunks} 分块 · {d.bytes.toLocaleString()} B</span>}
                        />
                      </List.Item>
                    )}
                  />
          ...
          <Col span={16}>
            ...
            >
              <Spin spinning={contentLoading}>
                {!selectedKey ? (
                  <Empty description="请在左侧选择文档" style={{ marginTop: 80 }} />
                ) : !isAdmin ? (
                  <div style={{ padding: 24, whiteSpace: 'pre-wrap', fontFamily: 'monospace', fontSize: 13, lineHeight: 1.8, height: '100%', overflow: 'auto' }}>
                    {content || '(文档内容为空)'}
                  </div>
                ) : (
                  <Input.TextArea
                    ...
                    style={{ height: '100%', resize: 'none', border: 'none', borderRadius: 0, fontFamily: 'monospace', fontSize: 13, lineHeight: 1.8 }}
