...
REIMBURSEMENT_POLICIES: list[PolicyRule] = [
    ...
    PolicyRule(
        ...
        level=RuleLevel.SOFT_LIMIT,
        title="招待费单次上限",
        condition={"expense_type": "entertainment"},
        limit=3000,
        unit="per_request",
    ...
    PolicyRule(
        rule_id="R301",
        level=RuleLevel.DEPARTMENTAL,
        title="研发部差旅专项额度（高）",
        condition={"expense_type": "travel"},
        limit=15000,
        unit="per_request",
        message="研发部差旅专项额度 ¥{limit:,.0f}，当前 ¥{amount:,.2f} 在范围内。",
        action=RuleAction.WARN,
        applicable_departments=["研发部"],
    ),
    PolicyRule(
        rule_id="R302",
        level=RuleLevel.DEPARTMENTAL,
